...
public:
    ...
    bool subscribe(std::function<void(T * const)>& subscriber) {
        if (std::find(std::begin(p_subscribers), std::end(p_subscribers), &subscriber) != std::end(p_subscribers))
            return false;
    ...
    bool unsubscribe(std::function<void(T * const)>& subscriber) {
        auto sub = std::find(std::begin(p_subscribers), std::end(p_subscribers), &subscriber);
